...
class NotificationWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        // 1. Prayer alerts — always check every 15 min
        checkAndNotifyPrayerPrecise(isAr)

        // 2. Gemini Daily Tip — throttled to once every 12 hours
        ...
        if (apiKey.isNotEmpty() && apiKey != "MY_GEMINI_API_KEY" && nowMs - lastGemini >= TWELVE_HOURS_MS) {
            try {
                ...
                val prompt = "Provide one very brief (12 words max) Islamic reminder or life tip in $promptLocale. Be direct. No intro."
                val res = model.generateContent(prompt).text ?: ""
                if (res.isNotBlank()) {
                    showNotification(
                        title = if (isAr) "تذكير يومي" else "Daily Reminder",
                        message = res.trim(),
                        id = 1002,
        ...
        // 3. Daily Quranic Wisdom — throttled to once every 24 hours
    ...
    private fun createNotificationChannels() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            val wisdomChannel = NotificationChannel(
                ...
                description = "حكمة يومية من القرآن الكريم وتذكير إسلامي"
                enableVibration(true)
    ...
    private fun showNotification(title: String, message: String, id: Int, channelId: String = CHANNEL_PRAYER) {
        ...
        val notification = NotificationCompat.Builder(applicationContext, channelId)
            ...
            .setCategory(
                if (channelId == CHANNEL_PRAYER) NotificationCompat.CATEGORY_ALARM
